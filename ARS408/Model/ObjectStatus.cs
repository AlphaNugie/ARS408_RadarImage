...
namespace ARS408.Model
{
    ...
    public class ObjectStatus : SensorMessage
    {
        ...
        private void DataConvert(string binary)
        {
            try
            {
                this.NofObjects = Convert.ToByte(binary.Substring(0, 8), 2);
                this.MeasCounter = Convert.ToUInt16(binary.Substring(8, 16), 2);
                this.InterfaceVersion = Convert.ToByte(binary.Substring(24, 4), 2);
            }
            catch (Exception) { }
        }
    }
